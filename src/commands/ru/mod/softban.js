...
module.exports = async (client,message,args,argsF) => {
	...
	const input = '\n\n\n<:tips:964246401485799514> **?softban <@ping> <reason>**\n\n**  └ <@ping> - пинг участника**\n\n**  └ <reason> - причина кика**'
	...
	if (!message.member.permissions.has("BAN_MEMBERS")) {
		...
		if (!user) {
	    	...
	    	let exampleEmbed = new MessageEmbed()
				.setColor(0x2f3136)
				.setDescription(`${lock} **Ой! Извините, но похоже, похоже, вы не указали участника!**${input}`)
			...
			if (mentionedRole.position > authorRole.position) {
				...
				if (mentionedRole.position == authorRole.position) {
					...
					if (message.guild.owner == user) {
				    	...
				    	let exampleEmbed = new MessageEmbed()
							.setColor(0x2f3136)
							.setDescription(`${lock} **Ой! Извините, но, похоже, ${user.username} является овнером и я не могу его забанить!**`)
						...
						if (user == message.author) {
					    	...
					    	let exampleEmbed = new MessageEmbed()
								.setColor(0x2f3136)
								.setDescription(`${lock} **Ой! Извините, но, похоже, вы не можете забанить себя!**`)
							...
							if (mentionedRole.position > botRole.position) {
								...
								try {
									...
									if (users.permissions.has('ADMINISTRATOR')) {
										...
										await users.ban()
								    	let exampleEmbed = new MessageEmbed()
								    		.setThumbnail('https://cdn.discordapp.com/emojis/1007618257345323018.png?v=1&size=48&quality=lossless')
											.setColor(0x00ffff)
							    	...
							    	let exampleEmbed = new MessageEmbed()
										.setColor(0x2f3136)
										.setDescription(`${lock} **Ой! Извините, но, похоже, я не могу забанить человка! Если у человка есть право администратор то я не смогу его забанить! Проверьте всё ли вы сделали правельно. Если да то обратитесь в тех поддержку!**${input}`)
