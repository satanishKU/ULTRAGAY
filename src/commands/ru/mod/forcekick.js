...
module.exports = async (client,message,args,argsF) => {
	...
	const input = '\n\n\n<:tips:964246401485799514> **?forcekick <@ping> <reason>**\n\n**  └ <@ping> - пинг участника**\n\n**  └ <reason> - причина кика**'

	if (!message.member.permissions.has("KICK_MEMBERS")) {

    	let exampleEmbed = new MessageEmbed()
			.setColor(0x2f3136)
			.setTitle('<:locked:1001383615625695292> Извините, но у вас нет прав!')
		...
		if (!user) {
			...
			if (mentionedRole.position > authorRole.position) {
		    	...
		    	let exampleEmbed = new MessageEmbed()
					.setColor(0x2f3136)
					.setDescription(`${lock} **Извините, но, похоже, ${user.username} выше вас!**`)
				...
				if (mentionedRole.position == authorRole.position) {
			    	let exampleEmbed = new MessageEmbed()
						.setColor(0x2f3136)
						.setDescription(`${lock} **Извините, но, похоже, ${user.username} на одном уровне с вами!**`)
					...
					if (message.guild.owner == user) {
				    	...
				    	let exampleEmbed = new MessageEmbed()
							.setColor(0x2f3136)
							.setDescription(`${lock} **Извините, но, похоже, ${user.username} является овнером и я не могу его кикнуть!**`)
						...
						if (user == message.author) {
					    	...
					    	let exampleEmbed = new MessageEmbed()
								.setColor(0x2f3136)
								.setDescription(`${lock} **Извините, но, похоже, вы не можете кикнуть себя!**`)
							...
							if (mentionedRole.position > botRole.position) {
						    	...
						    	let exampleEmbed = new MessageEmbed()
									.setColor(0x2f3136)
									.setDescription(`${lock} **Извините, но, похоже, ${user.username} выше меня!**`)
								...
								try {
									...
									if (users.permissions.has('ADMINISTRATOR')) {
								    	let exampleEmbed = new MessageEmbed()
											.setColor(0x2f3136)
											.setDescription(`${lock} **Извините, но, похоже, У человка есть право администратора! На него это не сработает!**`)
										...
										await users.kick()
								    	let exampleEmbed = new MessageEmbed()
								    		.setThumbnail('https://cdn.discordapp.com/emojis/1007619075867955250.png?v=1&size=48&quality=lossless')
											.setColor(0x00ffff)
											.setTitle('• × FORCE KICKED × •')
							    	...
							    	let exampleEmbed = new MessageEmbed()
										.setColor(0x2f3136)
										.setDescription(`${lock} **Извините, но, похоже, я не могу кикнуть человка! Если у человка есть право администратор то я не смогу его кикнуть! Проверьте всё ли вы сделали правельно. Если да то обратитесь в тех поддержку!**${input}`)
...
module.exports.names = ["?forcekick"];